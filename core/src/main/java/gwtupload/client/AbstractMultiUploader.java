...
public class AbstractMultiUploader<T extends IsWidget & HasWidgets.ForIsWidget> extends Composite implements IUploader {
     ...
     *
     * @return Status.INPROGRESS    if there are items being sent or queued.
    ...
    public void setEnabled(boolean isEnabled) {
        enabled = isEnabled;
        Set<IUploadStatus.CancelBehavior> cancel = isEnabled && !uploaders.isEmpty() ?
                currentUploader.getStatusWidget().getCancelConfiguration() :
                EnumSet.of(CancelBehavior.DISABLED);
        for (IUploader u : uploaders) {
            if (!u.equals(currentUploader)) {
                u.getStatusWidget().setCancelConfiguration(cancel);
            }
        }
        currentUploader.setEnabled(isEnabled);
    }
