...
public final class Utils {
    ...
    public static String getXmlNodeValue(NodeList list, String tagName, int idx) {
        ...
        final NodeList textNodes = node.getChildNodes();
        for (int i = 0; i < textNodes.getLength(); i++) {
            Node textNode = textNodes.item(i);
            if (textNode.getNodeType() == Node.TEXT_NODE
                    && !textNode.getNodeValue().replaceAll("[ \\n\\t\\r]", "").isEmpty()) {
                nodeValue.append(textNode.getNodeValue());
            } else if (textNode.getNodeType() == Node.CDATA_SECTION_NODE) {
                nodeValue.append(textNode.getNodeValue());
            }
        }
        return (nodeValue.length() == 0) ? null : nodeValue.toString().replaceAll("^\\s+", "").replaceAll("\\s+$", "");
    }
