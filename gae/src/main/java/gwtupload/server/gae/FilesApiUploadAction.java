...
public class FilesApiUploadAction extends UploadAction {
    ...
    public String executeAction(HttpServletRequest request, List<FileItem> sessionFiles) throws UploadActionException {
        final StringBuilder actionResponse = new StringBuilder();
        for (FileItem i : sessionFiles) {
            if (!i.isFormField()) {
                actionResponse.append((actionResponse.length() == 0) ? "" : " ")
                        .append(((FilesAPIFileItem) i).getKey().getKeyString());
                logger.info("Received new file, stored in blobstore with the key: " + actionResponse);
            }
        }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        final String blobKey = request.getParameter(PARAM_BLOBKEY);
        logger.info("Files doGet " + blobKey);
        if (blobKey != null) {
            logger.info("Serving a blobstore file with the key:" + blobKey);
            FilesAPIFileItem.getBlobstoreService().serve(new BlobKey(blobKey), response);
        } else {
